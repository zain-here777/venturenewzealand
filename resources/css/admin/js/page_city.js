(function ($) {
    ...
    $(document).on("click", ".city_edit", function () {
        ...
        let city_map_tile = $(this).attr('data-map_tile');
        ...
        if (city_map) {
            $('#preview_map').attr('src', `/uploads/city/map/${city_map}`);
        } else {
            $('#preview_map').attr('src', `https://via.placeholder.com/372x150?text=VentureNZ`);
        }
        if (city_map_tile) {
            $('#preview_map_tile').attr('src', `/uploads/city/map/${city_map_tile}`);
        } else {
            $('#preview_map_tile').attr('src', `https://via.placeholder.com/372x150?text=VentureNZ`);
        }
    ...
    $(document).on("click", ".city_delete", function () {
        if (confirm('Are you sure? The District that deleted can not restore!'))
            $(this).parent().submit();
    });
    ...
    $(document).on("change", ".city_is_popular", function () {
        let city_id = $(this).attr('data-id');
        let status = $(this).is(':checked');

        let data_resp = callAPI({
            url: getUrlAPI('/city/is_popular', 'api'),
            method: "put",
            body: {
                "city_id": city_id,
                "is_popular": status ? 1 : 0
            }
        });
        data_resp.then(res => {
            if (res.code === 200) {
    ...
    $('#city_map').change(function () {
        previewUploadImage(this, 'preview_map')
    });
    $('#city_map_tile').change(function () {
