...
$(document).ready(function() {
        ...
        if (window.innerWidth <= 1024) {
            ...
            $('#toggle_button').click(function() {
                if ($(this).text() === 'Read More') {
                    $('#country_description').css('max-height', 'none');
                    $(this).text('Show Less');
                } else {
                    $('#country_description').css('max-height', '250px');
                    $(this).text('Read More');
                }
            });
        } else {
            ...
            var golo_create_markers = function (data, map) {
                ...
                $.each(data, function (i, value) {
                    ...
                    if (value.categories[0].icon_map_marker) {
                        marker_options.icon = {
                            url: `/category/map-pin/${value.categories[0].id}`
                        }
                    }
    ...
    $(window).scroll(function (event) {
        ...
        if(width < 481){
            if(scroll>1100){
                $('.golo-menu-filter').css('position', 'fixed');
                $('.golo-menu-filter').css('top', '80px');
                $('.golo-menu-filter').css('width', '90%');
            }else{
                $('.golo-menu-filter').css('position', 'absolute');
                $('.golo-menu-filter').css('top', '0');
                $('.golo-menu-filter').css('width', '100%');
            }
        }
    });
// })(jQuery);
